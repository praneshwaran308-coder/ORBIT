...
(async () => {
  ...
  try {
    ...
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText.includes('Dispatch'));
      if (btn) btn.click();
    });
    ...
    fs.writeFileSync('C:/Users/LENOVO/.gemini/antigravity-ide/brain/d6e3ff2f-00a7-4458-bf0f-cde66affec7f/console_errors.txt', consoleErrors.join('\n'));
    console.log('PASS');
  } catch (e) {
