  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-surface-container-highest text-on-surface p-4 rounded shadow-lg max-w-md w-full flex flex-col md:flex-row items-center md:items-start space-y-2 md:space-y-0 md:space-x-4">
      ...
      <div className="flex space-x-2">
        <button onClick={accept} className="bg-primary text-on-primary px-3 py-1 rounded hover:bg-primary/80 transition">
          Accept
        </button>
        <button onClick={decline} className="bg-surface-container text-on-surface px-3 py-1 rounded hover:bg-surface-container-low transition">
          Decline
        </button>
      </div>
    </div>
